    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-body pt-4 p-3">
                <form action="{{ route('buat_spesifikasi') }}" method="POST" role="form text-left">
                    @csrf
                    @if($errors->any())
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="@error('tanggal')border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="date" placeholder="40770888444" id="number" name="tanggal" value="{{ auth()->user()->tanggal }}" value="{{ auth()->user()->noSurat }}" required>
                                        @error('tanggal')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                </div>
